...
def compute_fp_tp_probs_componentwise(
    ...
    """
    Modified compute_fp_tp_probs_nd:
    A detection (connected component) counts as True Positive (TP)
    if *any* pixel in the predicted component overlaps with the
    evaluation_mask. Otherwise, it is a False Positive (FP).

    Args:
        probs: array with shape (n,) containing the confidence scores
               for each detected component (e.g. max prob per component).
        component_masks: list of binary masks (same shape as evaluation_mask)
                         representing each predicted connected component.
        evaluation_mask: ground truth mask (binary or labeled) for evaluation.
        labels_to_exclude: labels in this list will not be counted for metric calculation.

    Returns:
        fp_probs: array containing probabilities of false positive detections.
        tp_probs: array containing probabilities of true positive detections
                  (aligned to GT labels if available).
        num_targets: total number of ground-truth targets considered.
    """
    ...
    # --- Determine ground truth target count ---
    max_label = int(np.max(evaluation_mask))
    ...
    # --- Evaluate each predicted component ---
    for p, comp_mask in zip(probs, component_masks):
        # If component overlaps with any GT region → TP
        overlap_labels = np.unique(evaluation_mask[comp_mask > 0])
        overlap_labels = [l for l in overlap_labels if l not in labels_to_exclude and l != 0]

        if len(overlap_labels) > 0:
            # Assign TP probability to corresponding GT label
            for l in overlap_labels:
                tp_probs[l - 1] = max(tp_probs[l - 1], p)
        else:
            # No overlap → FP
            fp_probs.append(p)

    fp_probs = np.array(fp_probs, dtype=np.float32)
...
def compute_fp_tp_probs(
    ...
    return compute_fp_tp_probs_componentwise(
